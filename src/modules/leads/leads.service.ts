...
import { Status, Lead } from '@prisma/client';
...
export class LeadsService {
    ...
    async update(id: string, updateLeadDto: UpdateLeadDto): Promise<Lead> {
        ...
        if (updateLeadDto.status === Status.LOST && !updateLeadDto.lostReason) {
        ...
        let nextFollowUpAt = updateLeadDto.nextFollowUpAt;
        if (updateLeadDto.status === Status.PROPOSAL && !nextFollowUpAt) {
            const now = new Date();
            nextFollowUpAt = new Date(now.getTime() + 2 * 24 * 60 * 60 * 1000).toISOString();
        }

        // Business rule: If status becomes WON, clear nextFollowUpAt
        if (updateLeadDto.status === Status.WON) {
            nextFollowUpAt = null;
        }
